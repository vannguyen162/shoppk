...
public class UserPdfExporter extends AbstractExporter {
	...
	public void export(List<User> listUsers, HttpServletResponse response) throws IOException {
		...
		Paragraph paragraph = new Paragraph("Danh sách thành viên", font);
		...
		table.setWidths(new int[] {2,3,3,4,3,3,3,4,2});
	...
	private void writeTableData(PdfPTable table, List<User> listUsers) {
		for(User user : listUsers) {
			...
			table.addCell(user.getIdentity_card());
			table.addCell(user.getEmail());
			table.addCell(user.getAdress());
			table.addCell(user.getRoles().toString());
			table.addCell(user.getDegrees().toString());
	...
	private void writeTableHeader(PdfPTable table) {
		...
		table.addCell(cell);
		cell.setPhrase(new  Phrase("Cmnd/Căn cước", font));
		table.addCell(cell);
		...
		table.addCell(cell);
		cell.setPhrase(new  Phrase("Bằng Cấp", font));
		table.addCell(cell);
