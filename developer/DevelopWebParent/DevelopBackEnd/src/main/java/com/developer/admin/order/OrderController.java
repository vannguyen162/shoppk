...
import com.developer.admin.product.ProductService;
import com.developer.admin.setting.SettingService;
...
public class OrderController {
	...
	public String listByPage(
			...
			@PathVariable(name = "pageNum") int pageNum, HttpServletRequest request, Model model) {
	...
	public String viewOrderDetails(@PathVariable(name = "id") Integer id, Model model, RedirectAttributes ra,
	...
	public String deleteOrder(@PathVariable Integer id, Model model, RedirectAttributes ra) {
		try {
			orderService.delete(id);
			ra.addFlashAttribute("message", "Đã xóa đơn hàng!");
		} catch (OrderNotFoundException ex) {
			ra.addFlashAttribute("message", ex.getMessage());
...
//		updateProductDetails(order, request);
	...
	private void updateProductDetails(Order order, HttpServletRequest request) {
		...
		for (int i = 0; i < detailIds.length; i++) {
			...
			updateProductQuantity(order, i, orderDetail);
		}
	}
	private void updateProductQuantity(Order order, int index, OrderDetail orderDetail) {
	    List<OrderTrack> orderTracks = order.getOrderTracks();
	    if (orderTracks.get(index).getStatus() == OrderStatus.PROCESSING) {
	        int quantity = orderDetail.getQuantity();
	        Product product = orderDetail.getProduct();
	        int remainingQty = product.getQty() - quantity;
