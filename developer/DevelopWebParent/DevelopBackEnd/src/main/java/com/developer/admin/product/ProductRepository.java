...
public interface ProductRepository extends PagingAndSortingRepository<Product, Integer> {
	...
	@Query("SELECT p FROM Product p WHERE p.kind = ?1 ORDER BY p.pdtCode DESC")
	List<Product> findByKindOrderByPdtCodeDesc(String kind);
	
	@Query("SELECT p FROM Product p WHERE p.enabled = true")
	List<Product> listAllProduct(String kind);
